...
TOKEN_SPEC = [
    ("NUMBER",   r"-?\d+(\.\d+)?|-?\d+"),  # Supports negative numbers (integers and floats)
    ("STRING",   r'"[^"]*"|\'[^\']*\''),
    ("ID",       r"[A-Za-z_]\w*"),
    ("OP",       r"[\+\-\*/=<>!]+"),
    ("NEWLINE",  r"\n"),
...
class Parser:
    ...
    def eat(self, typ):
        ...
        if tok and tok.type == typ:
            ...
            raise SyntaxError(f"Oczekiwano {typ}, dostano {tok.type if tok else 'EOF'}")
    ...
    def parse_factor(self):
        ...
        if tok.type == "NUMBER":
            ...
            raise SyntaxError(f"Nieoczekiwany token: {tok.type}")
