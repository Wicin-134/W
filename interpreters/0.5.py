...
class Var:
    ...
    def get_int(self):
        return int(self.s) if self.is_str else self.i

    def get_str(self):
        return self.s if self.is_str else str(self.i)
...
def cmd_math(expr):
    try:
        ...
        result = eval(left.strip(), {}, {k: v.get_int() for k, v in variables.items()})
        name = right.strip().strip("'")
        variables[name] = Var(result)
...
def cmd_random(args):
    try:
        ...
        variables[varname] = Var(random.randint(start, end))
    except Exception as e:
...
def cmd_if(expr):
    try:
        ...
        if tokens[0] == "int":
            ...
            op = tokens[2]
            val = int(tokens[3])
            var_val = variables.get(varname, Var()).get_int()
            if op == ">":
    ...
    if len(parts) == 2:
        ...
        try:
            with open(os.path.join(TMP_DIR, filename), "r") as f:
                variables[varname] = Var(f.read().strip())
        except Exception as e:
