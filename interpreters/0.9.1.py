...
class Parser:
    ...
    def eat(self, typ):
        ...
        if token and token.type == typ:
            self.pos += 1
            return token.value
        raise SyntaxError(f"Line {self.line_number}: expected {typ}, got {token.type if token else 'EOF'}")

    def parse_var(self):
        token = self.current()
        if DEBUG:
            print(f"[DEBUG] parse_var: current token = {token.type if token else 'None'}:{token.value if token else 'None'}")
    ...
    def parse_term(self):
        token = self.current()
        if not token:
            raise SyntaxError(f"Line {self.line_number}: unexpected end in term")
    ...
    def parse_array(self):
        if self.current() and self.current().type == 'STRING':
            values_str = self.eat('STRING').strip('"').strip("'")
            values = []
            for val in values_str.split(','):
                val = val.strip()
                if val and (val.replace('-', '').isdigit() or val.replace('.', '').replace('-', '').isdigit()):
                    values.append(Number(val).value)
                else:
                    values.append(StringNode(f'"{val}"').value)
            name = self.parse_var().name
        ...
        if token.type == 'ID':
            ...
            if cmd == 'show':
                ...
                if self.current() and self.current().type == 'OP' and self.current().value == '-':
                    ...
                    if self.current() and self.current().type == 'NUMBER':
                        number_val = '-' + self.eat('NUMBER')
                        return Assign(name, Number(number_val))
                    else:
                        raise SyntaxError(f"Line {self.line_number}: expected number after '-'")
                elif self.current() and self.current().type == 'NUMBER':
                    return Assign(name, Number(self.eat('NUMBER')))
                else:
                    expr = self.parse_expr()
                    return Assign(name, expr)
            elif cmd == 'bool':
                ...
                # expression or assignment like "x + 1 = x"
                self.pos -= 1
                expr = self.parse_expr()
                if self.current() and self.current().type == 'OP' and self.current().value == '=':
                    self.eat('OP')
...
def eval_cond(cond):
    ...
    if isinstance(cond, Cond):
        ...
        right = eval_cond(cond.right) if cond.op in ['and', 'or'] and hasattr(cond, 'right') else eval_expr(cond.right) if hasattr(cond, 'right') else None
        op = cond.op
        if op == 'and':
        ...
        if right is not None and op in ['=', '!=', '>', '<', '>=', '<=']:
            if isinstance(left, (int, float)) and isinstance(right, str) and right.replace('-', '').replace('.', '').isdigit():
                right = float(right) if '.' in right else int(right)
            elif isinstance(left, str) and isinstance(right, (int, float)) and left.replace('-', '').replace('.', '').isdigit():
...
def run_lines(lines, start_line=1):
    i = 0
        ...
        if line.startswith("func "):
            try:
                ...
                while i < len(lines):
                    ...
                    if stripped.startswith("done") and depth == 0:
                        break
                    if stripped.startswith("while ") or stripped.startswith("func "):
                        depth += 1
        ...
        if line.startswith("while "):
            try:
                ...
                while i < len(lines):
                    ...
                    line_number += 1
                if i >= len(lines):
                    print(f"[ERROR] Line {line_number}: missing 'done' for while")
                    i += 1
                    line_number += 1
                    continue
                whl.body = body
                run_node(whl, start_line_inner)
                i += 1
                line_number += 1
                continue
            except Exception as e:
...
def run_node(node, line_number=0):
    ...
    try:
        if isinstance(node, Cond):
            ...
            values = [v if isinstance(v, (int, float)) else v for v in node.values]
            if DEBUG:
            ...
            if node.body is None:
                raise RuntimeError(f"Line {line_number}: while body is missing")
            ...
            if node.name in functions:
                ...
                run_lines(functions[node.name], line_number)
            else:
            ...
            if os.path.isfile(path):
                with open(path, "r") as f:
                    content = f.read()
                if DEBUG:
                    print(f"[DEBUG] Read from '{filename}' content = {content}")
        ...
        elif isinstance(node, ClearAll):
            variables.clear()
            arrays.clear()
            arrays_str.clear()
            functions.clear()
            if DEBUG:
                print(f"[DEBUG] Cleared environment: variables/arrays/functions reset")
            print("[INFO] environment cleared")
        elif isinstance(node, ClearOutput):
            try:
                # ANSI clear + platform command fallback
                ...
                pass
            if DEBUG:
                print("[DEBUG] Cleared terminal output")
        elif isinstance(node, End):
            stop_program = True
        elif isinstance(node, (BinOp, Number, Var)):
...
# ---------------------------
CHANGELOG = """W Language v0.9.1 - Changelog (English)
- Bug fix: fixed parsing/execution of while loops inside functions (nested blocks and function-call execution).
- Added command `clear` to reset interpreter environment (clears variables, arrays and functions).
- Added command `clear-output` to clear the terminal output (works on Linux/macOS/Windows; ANSI fallback).
- No iteration-limit toggle included in v0.9.1 (MAX_ITERATIONS guard remains as a safety cap).
"""

CHEATSHEET = """W Language v0.9.1 - Cheatsheet
(Updated: added `clear` and `clear-output`)
...
clear           # clears variables, arrays, arrays_str and functions
clear-output    # clears terminal output (ANSI + platform command)
...
"""
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        if os.path.isfile(program):
            if DEBUG:
                print(f"[INFO] Running file: {program} with DEBUG = {DEBUG}")
            run_file(program)
        else:
